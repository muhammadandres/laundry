...
export default function Home() {
  ...
  const totalPendapatan = dashData.reduce(
    (sum, b) => sum + Number(b.total_bon),
    0
  );

  // Cek apakah filter di Diagram & Rekap aktif
  const isRekapFiltered = Boolean(
    fNama || fNomor || fTanggal || fBulan || fTahun
  );

  // Hitung Total Pendapatan Khusus Tab Karyawan
  const totalKarPendapatan = karData.reduce(
    (sum, b) => sum + Number(b.total_bon),
    0
  );
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">
                  Ringkasan performa dan transaksi bon laundry harian
                </p>
              </div>
              ...
              <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-black text-slate-900 mt-2">
                    ...
                    <span className="text-sm font-semibold text-slate-400 font-normal ml-1.5">
                      Bon
                    </span>
                  </p>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
                ...
                <div>
                  ...
                  <select
                    value={fBulan}
                    onChange={(e) => {
                      setFBulan(e.target.value);
                      setPage(1);
                    }}
                    className={inputFilterClass}
                    disabled={Boolean(fTanggal)}
                  >
                    {BULAN_OPTIONS.map((o) => (
                      <option key={o.val} value={o.val}>
                        {o.label}
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                <h2 className="text-lg font-bold text-slate-900">
                  Rekapitulasi Data Bon
                </h2>
              </div>
              {rekapLoading ? (
                <div className="bg-white rounded-2xl p-16 text-center text-slate-400 font-medium">
                  Memuat data tabel...
                </div>
              ) : (
                <TabelBon
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 flex flex-col md:flex-row md:items-end justify-between gap-4">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 flex-1">
                ...
                <div>
                  ...
                  <select
                    value={karBulan}
                    onChange={(e) => setKarBulan(e.target.value)}
                    className={inputFilterClass}
                  >
                    {BULAN_OPTIONS.map((o) => (
                      <option key={o.val} value={o.val}>
                ...
                <div>
                  ...
                  <select
                    value={karTahun}
                    onChange={(e) => setKarTahun(e.target.value)}
                    className={inputFilterClass}
                  >
                    <option value="">Semua Tahun</option>
                    {TAHUN_OPTIONS.map((t) => (
            ...
            <div className="bg-gradient-to-r from-blue-600 to-indigo-700 rounded-2xl p-6 text-white shadow-lg shadow-blue-500/20 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                <p className="text-xs font-bold uppercase tracking-wider text-blue-200">
                  Total Pendapatan Karyawan Terpilih
                </p>
              ...
              <div className="bg-white/10 backdrop-blur-md px-6 py-3 rounded-xl border border-white/20 text-right">
                ...
                <p className="text-2xl font-black text-white">
                  {formatRupiah(totalKarPendapatan)}
                </p>
              </div>
            </div>
              ...
              <div className="overflow-x-auto w-full">
                <table className="w-full text-sm text-left border-collapse">
                  <thead className="bg-slate-50 text-slate-500 uppercase text-[11px] font-bold tracking-wider border-b border-slate-100">
                    <tr>
                      <th className="px-4 sm:px-6 py-3.5 sm:py-4 whitespace-nowrap">
                        Nomor Bon
                      </th>
                      <th className="px-4 sm:px-6 py-3.5 sm:py-4 whitespace-nowrap">
                        Nama Karyawan
                      ...
                      <th className="px-4 sm:px-6 py-3.5 sm:py-4 text-right whitespace-nowrap">
                        Total Harga Bon
                      </th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100">
                    {karLoading ? (
                      <tr>
                        <td
                          colSpan="4"
                          className="px-4 sm:px-6 py-12 text-center text-slate-400 font-medium whitespace-nowrap"
                        >
                      ...
                      <tr>
                        <td
                          ...
                          className="px-4 sm:px-6 py-12 text-center text-slate-400 font-medium whitespace-nowrap"
                        >
                          Tidak ada transaksi bon pada kriteria ini.
                        </td>
                      </tr>
                    ) : (
                      karData.map((bon) => (
                        ...
                        >
                          ...
                          <td className="px-4 sm:px-6 py-3.5 sm:py-4 text-slate-700 font-medium whitespace-nowrap">
                            {bon.nama_karyawan}
                          </td>
                          <td className="px-4 sm:px-6 py-3.5 sm:py-4 text-slate-500 whitespace-nowrap">
                            {formatTanggal(bon.tanggal)}
                          </td>
                          <td className="px-4 sm:px-6 py-3.5 sm:py-4 text-right font-bold text-slate-900 whitespace-nowrap">
